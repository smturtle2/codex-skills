#!/usr/bin/env python3
"""Extract rembg-normalized animation sheets into transparent frames."""
    ...
    for start, value in enumerate(data):
        ...
        while stack:
            ...
            for nx, ny in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
                ...
                if visited[neighbor] or data[neighbor] <= 16:
                    continue
                visited[neighbor] = 1
...
def state_layout(state: dict[str, Any], frame_count: int) -> dict[str, int]:
    ...
    if layout.get("safe_margin_x") is not None:
        result["safe_margin_x"] = int(layout["safe_margin_x"])
    if layout.get("safe_margin_y") is not None:
        result["safe_margin_y"] = int(layout["safe_margin_y"])
    return result
...
def slot_boxes(sheet: Image.Image, layout: dict[str, int], frame_count: int) -> list[tuple[int, int, int, int]]:
    return [
        (
            round((index % layout["columns"]) * sheet.width / layout["columns"]),
            round((index // layout["columns"]) * sheet.height / layout["rows"]),
            round((index % layout["columns"] + 1) * sheet.width / layout["columns"]),
            round((index // layout["columns"] + 1) * sheet.height / layout["rows"]),
        )
...
def reject_guide_like_components(components: list[dict[str, Any]], sheet: Image.Image, layout: dict[str, int]) -> None:
    slot_w = sheet.width / layout["columns"]
    slot_h = sheet.height / layout["rows"]
    for component in components:
        ...
        if width <= 0 or height <= 0:
            continue
        thin = min(width, height) / max(width, height) < 0.08
        spans_sheet = width > sheet.width * 0.75 or height > sheet.height * 0.75
        spans_slots = width > slot_w * 1.35 or height > slot_h * 1.35
        if thin and (spans_sheet or spans_slots):
            raise SystemExit("rembg output still contains visible grid, guide, or border lines; regenerate the action sheet")


def reject_visible_guide_lines(sheet: Image.Image) -> None:
    rgba = sheet.convert("RGBA")
    pixels = rgba.load()
    row_threshold = max(24, round(rgba.width * 0.45))
    column_threshold = max(24, round(rgba.height * 0.45))
...
def extract_component_frames(
    ...
    reject_visible_guide_lines(sheet)
    components = connected_components(sheet)
    reject_guide_like_components(components, sheet, layout)
    if not components:
        return None
...
def scaled_safe_box(
    ...
    safe_x = state.get("safe_margin_x", layout.get("safe_margin_x"))
    safe_y = state.get("safe_margin_y", layout.get("safe_margin_y"))
...
def main() -> None:
    ...
    settings = filter_states(
        manifest_settings(manifest, frame_size=frame_size, fps=args.fps),
        args.action_id,
    )
    ...
    for state in settings["states"]:
        ...
        if not source.is_file():
            raise SystemExit(f"missing rembg-normalized sheet for {state['name']}: {source}")
        ...
        if args.method in {"auto", "components"}:
            frames = extract_component_frames(sheet, int(state["frames"]), layout, crop_box_values, generated_frame_size)
            if frames is None:
                raise SystemExit(f"could not extract {state['frames']} foreground components from rembg output {source}")
            used_method = "components"
        if frames is None:
            frames, crop_boxes = extract_slot_frames(sheet, int(state["frames"]), layout, state, crop_box_values, generated_frame_size)
            used_method = "slots"
        ...
        rows.append(
            {
                ...
                "method": used_method,
                "layout": state["layout"],
                "extracted_frame_size": list(frames[0].size) if frames else [],
                "extraction_size_policy": "preserve-generated-cell-size",
                "background_removal": "required-rembg-before-extraction",
